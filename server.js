...
// Auth helpers (simplified for brevity)
...
function isAdmin(req, res, next) { const user = getUserFromToken(req); if (user && user.discordId === ADMIN_ID) { req.user = user; return next(); } res.status(403).send('Forbidden'); }
...
async function calculateProgress(stats, tiers) {
  return stats.map(stat => {
    ...
    // Calculate requirements using multipliers
    const killReq = Math.floor(power * Number(tier.kill_multiplier));
    const deathReq = Math.floor(power * Number(tier.death_multiplier)); // NOW PERCENTAGE BASED
...
app.get('/stats', async (req, res) => {
  try {
    const stats = await getAllStats(); const tiers = await getAllTiers(); const statsWithProgress = await calculateProgress(stats, tiers);
    res.render('stats', { stats: statsWithProgress, tiers });
  } catch (error) { res.status(500).send('Error'); }
});

app.get('/auth/discord', (req, res) => {
  res.redirect(`https://discord.com/api/oauth2/authorize?client_id=${process.env.DISCORD_CLIENT_ID}&redirect_uri=${encodeURIComponent(process.env.DISCORD_CALLBACK_URL)}&response_type=code&scope=identify`);
});
...
app.get('/auth/discord/callback', async (req, res) => {
  ...
  try {
    const tr = await fetch('https://discord.com/api/oauth2/token', {
      ...
      body: new URLSearchParams({ client_id: process.env.DISCORD_CLIENT_ID, client_secret: process.env.DISCORD_CLIENT_SECRET, grant_type: 'authorization_code', code, redirect_uri: process.env.DISCORD_CALLBACK_URL })
    });
    ...
    const token = jwt.sign({ discordId: ud.id, username: ud.username, avatar: ud.avatar, isAdmin: ud.id === ADMIN_ID }, JWT_SECRET, { expiresIn: '7d' });
...
app.get('/admin', isAuthenticated, isAdmin, async (req, res) => {
  try { const stats = await getAllStats(); const tiers = await getAllTiers(); res.render('admin', { user: req.user, stats, tiers }); } catch { res.status(500).send('Error'); }
});

app.post('/admin/tier', isAuthenticated, isAdmin, async (req, res) => {
  try {
    const { id, name, minPower, maxPower, killMultiplier, deathMultiplier } = req.body;
    await upsertTier(id || null, name, parseInt(minPower), parseInt(maxPower), parseFloat(killMultiplier), parseFloat(deathMultiplier));
...
app.post('/admin/tier/delete', isAuthenticated, isAdmin, async (req, res) => {
  try { await deleteTier(req.body.id); res.redirect('/admin?success=tier_deleted'); } catch { res.redirect('/admin?error=delete_failed'); }
});

app.post('/admin/upload/creation', isAuthenticated, isAdmin, upload.single('file'), async (req, res) => {
  try {
    if (!req.file) return res.status(400).send('No file'); const records = parseExcelData(req.file.buffer);
    if (!records.length) return res.status(400).send('No records'); await clearAllStats();
    for (const r of records) await createStatsWithInitial(r.governorId, r.username, r.highestPower, r.deads, r.killPoints, r.resources);
    res.status(200).send(`Creation complete! ${records.length} records`);
...
app.post('/admin/upload/update', isAuthenticated, isAdmin, upload.single('file'), async (req, res) => {
  try {
    if (!req.file) return res.status(400).send('No file'); const records = parseExcelData(req.file.buffer);
    if (!records.length) return res.status(400).send('No records');
    for (const r of records) await upsertStats(r.governorId, r.username, r.highestPower, r.deads, r.killPoints, r.resources);
    res.status(200).send(`Update complete! ${records.length} records`);
  } catch (e) { res.status(500).send('Failed: ' + e.message); }
});

app.get('/api/stats', async (req, res) => {
  try { const stats = await getAllStats(); const tiers = await getAllTiers(); const sp = await calculateProgress(stats, tiers); res.json(sp); } catch { res.status(500).json({ error: 'Error' }); }
});
